...
## v7.6.6
- Editable unit cost and minimum-stock fields on every inventory row.
- Quick + / − stock adjustments plus full stocktake mode.
- Build Capacity now lists the exact snare size, depth and construction combinations possible from available hardware.
- Reorder Planner compares a user-editable target drum mix against available stock and orders only shortages.
- Default target: 6 × 14×6½, 4 × 14×5½, 4 × 13×7, 4 × 12×7 and 4 × 10×5.
- Approximate purchase-order value uses editable landed unit costs.
- Migration v7_6_6 seeds estimated AUD costs only where the existing cost is blank or zero.
