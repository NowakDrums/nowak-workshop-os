...
function App(){
  ...
  return <main>
    <header className="hero">
      <div className="heroBrand">
        <img src={nowakLogo} alt="Nowak Drum Company Australia" className="nowakHeaderLogo"/>
        <div><h1>Nowak Workshop OS</h1><p>v7.9.13 — scheduled repair dates and flexible purchase-order suppliers.</p></div>
      </div>
      <button onClick={loadAll}><RefreshCw size={16}/> Refresh</button>
    </header>
...
function Inventory({hardware, allocations=[], drums=[], updateHardware, saveStocktake, refreshData, allocateHardware, releaseHardware, lowStock, inventoryValue}){
  ...
  const supplierGroupNames=[...new Set(orderRows.map(row=>supplierBucket(row.part,row.preferredSupplier)))];
  const supplierOrderGroups=supplierGroupNames.map(supplier=>{
  ...
  const otherSupplierGroups=supplierOrderGroups.filter(group=>group.supplier!=="Lea Hung");
