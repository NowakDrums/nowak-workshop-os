...
function Inventory({hardware, allocations=[], drums=[], updateHardware, saveStocktake, refreshData, allocateHardware, releaseHardware, lowStock, inventoryValue}){
  ...
  // Carry current allocation shortages into the next purchase-order calculation.
  ...
  const purchaseSupplierName=part=>{
    const supplier=String(part?.supplier||"").trim();
    if(/lea\s*hung/i.test(supplier)) return "Lea Hung";
    if(/mega\s*music|mega/i.test(supplier)) return "Mega Music";
    if(/rech/i.test(supplier)) return "Rech";
    return supplier||"Other Supplier";
  };
  const purchaseSupplierOrder=["Lea Hung","Mega Music","Rech","Other Supplier"];
  const supplierOrderGroups=purchaseSupplierOrder
    .map(supplier=>{
      const rows=orderRows
        .filter(row=>purchaseSupplierName(row.part)===supplier)
      ...
      return {supplier,rows,estimate:rows.reduce((sum,row)=>sum+row.total,0)};
    })
  ...
  return <section className="panel inventoryModule">
    ...
    {activeTab==="stock"&&<><div className="inventorySummaryGrid"><article className="card"><b>On hand</b><strong>{visibleHardware.reduce((s,p)=>s+Number(p.qty_on_hand||0),0)}</strong></article><article className="card"><b>Allocated</b><strong>{visibleHardware.reduce((s,p)=>s+Number(p.qty_allocated||0),0)}</strong></article><article className="card"><b>Available</b><strong>{visibleHardware.reduce((s,p)=>s+Number(p.qty_available||0),0)}</strong></article><article className="card"><b>Stock value</b><strong>{money(inventoryValue)}</strong></article></div>
      ...
      {groups.map(group=>{const parts=(stocktakeMode?stocktakeParents:visibleHardware).filter(p=>p.category===group).sort(sortParts);if(!parts.length)return null;return <section className="inventoryBlock" key={group}><h3>{group}</h3><div className="tableWrap"><table><thead><tr><th>Part</th><th>Supplier</th><th>Code / size</th><th>On hand</th><th>Allocated</th><th>Available</th><th>Unit cost</th><th>Minimum</th></tr></thead><tbody>{parts.flatMap(p=>{
        ...
        const isEmptyAllocated=Number(p.qty_available||0)===0&&Number(p.qty_allocated||0)>0;
        const needsOrder=isShort||isEmptyAllocated;
        const main=<tr className={needsOrder?"inventoryShortageRow":""} key={p.id}><td>{p.part_name}{p.finish&&<><br/><small>{p.finish}</small></>}{needsOrder&&<span className="shortageBadge">{isShort?`SHORT ${Math.abs(Number(p.qty_available||0))}`:"ZERO AVAILABLE"}</span>}</td><td>{p.supplier||"—"}</td><td>{p.code}<br/><small>{p.size}</small></td><td>{stocktakeMode?<input className="compactInput" type="number" min="0" inputMode="numeric" data-stock-id={p.id} defaultValue={Number(p.qty_on_hand||0)}/>:<b>{p.qty_on_hand}</b>}</td><td><b>{p.qty_allocated}</b>{Number(p.qty_allocated||0)>0&&<small className="allocationHint">Reserved for {p.allocated_drum_ids?.length||0} drum{(p.allocated_drum_ids?.length||0)===1?"":"s"}</small>}</td><td><b className={needsOrder?"dangerText":""}>{p.qty_available}</b>{needsOrder&&<small className="shortageHint">Order at least {isShort?Math.abs(Number(p.qty_available||0)):1}</small>}</td><td>{stocktakeMode?<input className="moneyInput" type="number" min="0" step="0.01" defaultValue={Number(p.landed_cost_aud||0)} onBlur={e=>Number(e.target.value)!==Number(p.landed_cost_aud||0)&&updateHardware(p.id,{landed_cost_aud:Number(e.target.value||0)})}/>:money(p.landed_cost_aud||0)}</td><td>{stocktakeMode?<input className="compactInput" type="number" min="0" data-stock-min-id={p.id} defaultValue={Number(p.reorder_level||0)}/>:<b>{p.reorder_level||0}</b>}</td></tr>;
