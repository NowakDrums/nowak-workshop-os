...
function App(){
  ...
  async function syncHardwareUsed(drum,selectedRequirements,consumeNow=false){
    ...
    // Stock drums must not keep a reservation: the returned part becomes fully available.
    // Custom orders may keep the part reserved for later fitting.
    const isCustomOrder=String(drum.sales_status||"").toLowerCase()==="custom order" || String(drum.order_type||"").toLowerCase()==="custom";
    ...
    // If no hardware is fitted, stock drums should have no active allocation at all.
    // Custom orders may keep the reservation because those parts are intentionally
    // set aside for that customer.
    if(selectedRequirements.length===0){
      ...
      if(activeReservationIds.length){
        let releaseReservation=!isCustomOrder;
        if(isCustomOrder){
          releaseReservation=window.confirm(
            "All fitted hardware has been returned to stock.\n\nRelease the hardware reservation from this custom order as well?\n\nOK = release it to general available stock.\nCancel = keep it reserved for this custom order."
          );
        }
  ...
  return <main>
    ...
    {view==="inventory" && <Inventory hardware={availableHardware} allocations={hardwareAllocations} drums={drums} openJobCard={setJobCard} updateHardware={updateHardware} saveStocktake={saveHardwareStocktake} refreshData={loadAll} allocateHardware={allocateHardwareForDrum} releaseHardware={releaseHardwareForDrum} lowStock={lowStock} inventoryValue={inventoryValue}/>}
...
function Inventory({hardware, allocations=[], drums=[], openJobCard, updateHardware, saveStocktake, refreshData, allocateHardware, releaseHardware, lowStock, inventoryValue}){
  ...
  const openAllocatedDrums=(ids=[])=>{
    const linked=[...new Set((ids||[]).filter(Boolean))].map(id=>drums.find(d=>String(d.id)===String(id))).filter(Boolean);
    if(!linked.length)return;
    if(linked.length===1){openJobCard?.(linked[0]);return;}
    const choices=linked.map((d,index)=>`${index+1}. #${d.serial||"Pending"} · ${d.timber||"Timber not entered"} · ${d.size||"Size not entered"}${d.customer?` · ${d.customer}`:""}`).join("\n");
    const answer=window.prompt(`This hardware is reserved for ${linked.length} drums. Enter the number of the Job Card to open:\n\n${choices}`);
    if(answer===null)return;
    const selected=linked[Number(answer)-1];
    if(selected)openJobCard?.(selected);
  };
  ...
  const displayHardware=Array.from(hardware.reduce((map,part)=>{
    ...
    map.set(key,{
      ...
      allocated_drum_ids:[...new Set([...(existing.allocated_drum_ids||[]),...(part.allocated_drum_ids||[])])],
  ...
  return <section className="panel inventoryModule">
    ...
    {activeTab==="stock"&&<><div className="inventorySummaryGrid"><article className="card"><b>On hand</b><strong>{visibleHardware.reduce((s,p)=>s+Number(p.qty_on_hand||0),0)}</strong></article><article className="card"><b>Allocated</b><strong>{visibleHardware.reduce((s,p)=>s+Number(p.qty_allocated||0),0)}</strong></article><article className="card"><b>Available</b><strong>{visibleHardware.reduce((s,p)=>s+Number(p.qty_available||0),0)}</strong></article><article className="card"><b>Stock value</b><strong>{money(inventoryValue)}</strong></article></div>
      ...
      {groups.map(group=>{const parts=(stocktakeMode?stocktakeParents:visibleHardware).filter(p=>p.category===group).sort(sortParts);if(!parts.length)return null;return <section className="inventoryBlock" key={group}><h3>{group}</h3><div className="tableWrap"><table><thead><tr><th>Part</th><th>Supplier</th><th>Code / size</th><th>On hand</th><th>Allocated</th><th>Available</th><th>Unit cost</th><th>Minimum</th></tr></thead><tbody>{parts.flatMap(p=>{
        ...
        const main=<tr className={rowClass} key={p.id}><td>{p.part_name}{p.finish&&<><br/><small>{p.finish}</small></>}{isShort&&<span className="shortageBadge">{`SHORT ${Math.abs(Number(p.qty_available||0))}`}</span>}{hasAllocation&&!isShort&&<span className="allocationBadge">ALLOCATED</span>}</td><td>{p.supplier||"—"}</td><td>{p.code}<br/><small>{p.size}</small></td><td>{stocktakeMode?<input className="compactInput" type="number" min="0" inputMode="numeric" data-stock-id={p.id} defaultValue={Number(p.qty_on_hand||0)}/>:<b>{p.qty_on_hand}</b>}</td><td>{hasAllocation&&!stocktakeMode?<button type="button" className="allocationOpenButton" onClick={()=>openAllocatedDrums(p.allocated_drum_ids||[])} title="Open reserved drum Job Card"><b>{p.qty_allocated}</b><small className="allocationHint">Reserved for {p.allocated_drum_ids?.length||0} drum{(p.allocated_drum_ids?.length||0)===1?"":"s"} · Open</small></button>:<><b>{p.qty_allocated}</b>{hasAllocation&&<small className="allocationHint">Reserved for {p.allocated_drum_ids?.length||0} drum{(p.allocated_drum_ids?.length||0)===1?"":"s"}</small>}</>}</td><td><b className={isShort?"dangerText":hasAllocation?"okText":""}>{p.qty_available}</b>{isShort&&<small className="shortageHint">Order at least {Math.abs(Number(p.qty_available||0))}</small>}</td><td>{stocktakeMode?<input className="moneyInput" type="number" min="0" step="0.01" defaultValue={Number(p.landed_cost_aud||0)} onBlur={e=>Number(e.target.value)!==Number(p.landed_cost_aud||0)&&updateHardware(p.id,{landed_cost_aud:Number(e.target.value||0)})}/>:money(p.landed_cost_aud||0)}</td><td>{stocktakeMode?<input className="compactInput" type="number" min="0" data-stock-min-id={p.id} defaultValue={Number(p.reorder_level||0)}/>:<b>{p.reorder_level||0}</b>}</td></tr>;
        if(!stocktakeMode) return [main];
        const existingRows=variants.map(v=>{const allowMinimum=String(v.category||"")==="Snare Wires";const variantShort=Number(v.qty_available||0)<0;const variantAllocated=Number(v.qty_allocated||0)>0;return <tr className={`variantRow ${variantShort?"inventoryShortageRow":variantAllocated?"inventoryAllocatedRow":""}`} key={v.id}><td><span className="variantIndent">↳ {v.finish||v.part_name}</span>{variantShort&&<span className="shortageBadge">SHORT {Math.abs(Number(v.qty_available||0))}</span>}{variantAllocated&&!variantShort&&<span className="allocationBadge">ALLOCATED</span>}</td><td>{v.supplier||"—"}</td><td>{v.code}<br/><small>{v.size}</small></td><td><input className="compactInput" type="number" min="0" inputMode="numeric" data-stock-id={v.id} defaultValue={Number(v.qty_on_hand||0)}/></td><td>{variantAllocated&&!stocktakeMode?<button type="button" className="allocationOpenButton" onClick={()=>openAllocatedDrums(v.allocated_drum_ids||[])} title="Open reserved drum Job Card"><b>{v.qty_allocated}</b><small className="allocationHint">Reserved · Open</small></button>:<><b>{v.qty_allocated}</b>{variantAllocated&&<small className="allocationHint">Reserved</small>}</>}</td><td><b className={variantShort?"dangerText":variantAllocated?"okText":""}>{v.qty_available}</b></td><td>{money(v.landed_cost_aud||0)}</td><td>{allowMinimum?<input className="compactInput" type="number" min="0" data-stock-min-id={v.id} defaultValue={Number(v.reorder_level||0)}/>:<b>—</b>}</td></tr>});
        const missingRows=[];
...
function SocialPostCreator({drum,media,onClose,setMessage}){
  ...
  return <div className="socialPostCreatorOverlay">
    <style>{`
      ...
      .allocationHint{display:block;color:#d4a94e;font-size:11px;margin-top:3px}
.allocationOpenButton{display:flex;flex-direction:column;align-items:flex-start;gap:1px;width:100%;padding:5px 7px;background:transparent;border:1px solid rgba(95,211,146,.35);border-radius:8px;color:inherit;text-align:left;cursor:pointer}.allocationOpenButton:hover{border-color:#5fd392;background:rgba(95,211,146,.08)}.allocationOpenButton b{color:#5fd392}.reservedDrumLinks{display:flex;gap:6px;flex-wrap:wrap}.reservedDrumLinks button{padding:4px 8px;border-radius:999px;border:1px solid rgba(95,211,146,.45);background:rgba(95,211,146,.08);color:#5fd392;font-weight:700;cursor:pointer}
...
function JobCard({drum, template, labourRate, onClose, updateDrum, completeDrum, addTime, markSold, markShipped, returnDrumToProduction, archiveDrum, restoreArchivedDrum, setDrumLifecycle, copyText, deleteDrum, drums=[], projects=[], createProject, setMessage, onAddDrumToProject, hardware=[], hardwareAllocations=[], consumeHardwareForDrum, syncHardwareUsed,releaseHardwareForDrum}){
  ...
  async function saveAllChanges(){
    ...
    // A NULL lifecycle_status is meaningful: it means the drum is back in production.
    // Do not fall back to the stale Job Card lifecycle, otherwise Undo Complete can
  ...
  return <div className="modalBg" onClick={onClose}><div className={"modal jobModal "+(drum.build_client==="Brady"?"bradyModal":"")} onClick={e=>e.stopPropagation()}>
    ...
    {standardHardware.length>0 && <section className="hardwareUsedBar">
      <div><b>Hardware fitted / used</b><span>{consumedForDrum.length
        ? `${consumedForDrum.reduce((sum,row)=>sum+Number(row.quantity||0),0)} pieces currently recorded as fitted. Deselect a part to return it to stock. Stock drums release it completely; custom orders can keep it reserved.`
