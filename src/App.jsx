...
function App(){
  ...
  return <main>
    <header className="hero">
      <div className="heroBrand">
        <img src={nowakLogo} alt="Nowak Drum Company Australia" className="nowakHeaderLogo"/>
        <div><h1>Nowak Workshop OS</h1><p>v7.9.27 — printable stocktake, purchase orders and kit hardware breakdowns.</p></div>
      </div>
      <button onClick={loadAll}><RefreshCw size={16}/> Refresh</button>
    </header>
...
${(order.order_items||[]).map(i=>`${i.name} | ${i.colour} | ${i.code} | ${i.size} | Qty ${i.quantity}`).join("\n")}`;
  ...
  const printKitHardwareBreakdown=()=>{
    ...
    const drumSections=planRows.map((row,index)=>{
      ...
      const reqs=requirementsFor(row.diameter,row.depth,row.buildType,type,finish);
      const itemRows=reqs.map(req=>{
        const lineQty=Number(req.qty||0)*drumQty;
        const totalKey=[req.code||req.label,hardwareFinishLabel(req.finish||req.label||finish),req.size||""].join("|");
        const existing=totals.get(totalKey)||{category:hardwareBreakdownCategory(req),name:req.partName||req.label||req.code,finish:hardwareFinishLabel(req.finish||req.label||finish),size:req.size||"",code:req.code||"",qty:0};
        existing.qty+=lineQty;
        totals.set(totalKey,existing);
        return `<tr><td>${escapeHtml(req.partName||req.label||req.code)}</td><td>${escapeHtml(hardwareFinishLabel(req.finish||req.label||finish))}</td><td>${escapeHtml(req.size||"")}</td><td>${Number(req.qty||0)}</td><td>${drumQty}</td><td><b>${lineQty}</b></td></tr>`;
      }).join("");
      return `<section class="drum"><h2>${index+1}. ${drumQty} × ${escapeHtml(row.diameter)} × ${escapeHtml(row.depth)} ${escapeHtml(type)} — ${escapeHtml(row.buildType||"")} — ${escapeHtml(finish)}</h2><table><thead><tr><th>Hardware item</th><th>Finish</th><th>Size</th><th>Per drum</th><th>Drums</th><th>Line total</th></tr></thead><tbody>${itemRows}</tbody></table></section>`;
    }).join("");
    ...
    const totalItems=[...totals.values()].sort((a,b)=>orderedCategories.indexOf(a.category)-orderedCategories.indexOf(b.category)||String(a.name).localeCompare(String(b.name)));
    const totalsHtml=orderedCategories.map(category=>{
      const rows=totalItems.filter(item=>item.category===category);
      if(!rows.length)return "";
      return `<tr class="category"><td colspan="5">${escapeHtml(category)}</td></tr>${rows.map(item=>`<tr><td>${escapeHtml(item.name)}</td><td>${escapeHtml(item.finish)}</td><td>${escapeHtml(item.size)}</td><td>${escapeHtml(item.code)}</td><td><b>${item.qty}</b></td></tr>`).join("")}`;
    }).join("");
    const html=`<!doctype html><html><head><title>Nowak Kit Hardware Breakdown</title><style>@page{size:A4 landscape;margin:10mm}body{font-family:Arial,Helvetica,sans-serif;color:#111;font-size:10px}h1{margin:0 0 4px;font-size:20px}h2{font-size:13px;margin:16px 0 6px}.intro{margin:0 0 12px}.drum{break-inside:avoid;margin-bottom:14px}table{width:100%;border-collapse:collapse;margin-bottom:8px}th,td{border:1px solid #888;padding:5px;text-align:left;vertical-align:top}th{background:#ddd}.category td{background:#eee;font-weight:700;font-size:11px}.totalTitle{margin-top:22px;border-top:2px solid #111;padding-top:10px}</style></head><body><h1>Nowak Drum Company — Kit Hardware Breakdown</h1><p class="intro">Printed ${new Date().toLocaleString("en-AU")} · ${planTotal} drum${planTotal===1?"":"s"} in this build plan.</p>${drumSections}<h1 class="totalTitle">Combined Hardware Totals</h1><table><thead><tr><th>Hardware item</th><th>Finish</th><th>Size</th><th>Code</th><th>Total required</th></tr></thead><tbody>${totalsHtml}</tbody></table></body></html>`;
    openPrintableDocument(html,"Please allow pop-ups to print or save the kit hardware breakdown.");
  };
