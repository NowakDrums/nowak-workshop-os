...
function Inventory({hardware, allocations=[], drums=[], updateHardware, saveStocktake, allocateHardware, releaseHardware, lowStock, inventoryValue}){
  ...
  const buildableOptions=[];snareInventoryDiameters.forEach(d=>snareInventoryDepths.forEach(depth=>["Stave","Ply"].forEach(type=>{if((d==="12"||d==="13")&&depth==="8")return;const qty=capacityFor(d,depth,type);if(qty>0)buildableOptions.push({d,depth,type,qty})})));
  ...
  while(mixProgress){
    ...
    simultaneousMix.forEach(row=>{
      ...
      const reqs=requirementsFor(row.diameter,row.depth,row.buildType);
  ...
  const groupedLeaHungRows=()=>{
    const ordered=["Lugs","Air Vents","Tension Rods","Hoops","Snare Wires"];
  ...
  const copyLeaHungEmail=async()=>{
    try{
      if(navigator.clipboard?.write && typeof ClipboardItem!=="undefined"){
        ...
        setSupplierOrderMessage("Formatted Lea Hung email copied. Paste it into your email draft.");
      }else{
        await navigator.clipboard?.writeText(`To: contact@leahung.com\nSubject: Hardware Order\n\n${leaHungEmailBody()}`);
        setSupplierOrderMessage("Lea Hung order email copied as plain text.");
      }
    }catch{
      await navigator.clipboard?.writeText(`To: contact@leahung.com\nSubject: Hardware Order\n\n${leaHungEmailBody()}`);
      setSupplierOrderMessage("Lea Hung order email copied as plain text.");
    }
    window.setTimeout(()=>setSupplierOrderMessage(""),3500);
  };
